...
import com.sap.copy.archcomp.SingleLiveEvent;
import com.sap.copy.mediaresource.EntityMediaResource;

import com.sap.copy.repository.OperationResult.Operation;
...
public class Repository<T extends EntityValue> {
     ...
     * OData service
     */
    private EAM_NTF_CREATE_Entities eAM_NTF_CREATE_Entities;
     ...
     * @param eAM_NTF_CREATE_Entities - OData service
     * @param entitySet - the entity set this repository provides access for
     */
    public Repository(@NonNull EAM_NTF_CREATE_Entities eAM_NTF_CREATE_Entities, @NonNull EntitySet entitySet, @Nullable Property orderByProperty) {
        this.eAM_NTF_CREATE_Entities = eAM_NTF_CREATE_Entities;
        this.entitySet = entitySet;
        this.orderByProperty = orderByProperty;
        observableEntities = new MutableLiveData<>();
        if (EntityMediaResource.isV4(eAM_NTF_CREATE_Entities.getMetadata().getVersionCode()) && EntityMediaResource.hasMediaResources(entitySet)) {
    ...
    public void read() {
        ...
        eAM_NTF_CREATE_Entities.executeQueryAsync(dataQuery,
            result -> {
    ...
    public void read(EntityValue parent, String navigationPropertyName) {
        ...
        eAM_NTF_CREATE_Entities.loadPropertyAsync(navigationProperty, parent, dataQuery,
            () -> {
    ...
    public void create(@NonNull T newEntity, @NonNull StreamBase media) {
        if (newEntity.getEntityType().isMedia()) {
            eAM_NTF_CREATE_Entities.createMediaAsync(newEntity, media,
                () -> {
    ...
    public void create(@NonNull T newEntity) {
        ...
        eAM_NTF_CREATE_Entities.createEntityAsync(newEntity,
            () -> {
                insertToCache(newEntity, entities);
                if (relatedEntities.size() > 0) {
                    insertToCache(newEntity, relatedEntities);
    ...
    public void update(@NonNull T updateEntity) {
        eAM_NTF_CREATE_Entities.updateEntityAsync(updateEntity,
            () -> {
    ...
    public void delete(@NonNull List<T> deleteEntities) {
        ...
        eAM_NTF_CREATE_Entities.applyChangesAsync(deleteChangeSet,
            () -> {
