...
import com.sap.copy.databinding.FragmentNotesetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing Note entity.
 * This fragment is contained in the {@link NoteSetActivity}.
 */
public class NoteSetCreateFragment extends InterfacedFragment<Note> {

    private static final Logger LOGGER = LoggerFactory.getLogger(NoteSetCreateFragment.class);
    ...
    /** Note object and it's copy: the modifications are done on the copied object. */
    private Note noteEntity;
    private Note noteEntityCopy;

    /** DataBinding generated class */
    private FragmentNotesetCreateBinding binding;
    ...
    /**
     * This fragment is used for both update and create for NoteSet to enter values for the properties.
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.note.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.note.getLocalName();
        }

        ((NoteSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            ...
            noteEntity = viewModel.getSelectedEntity().getValue();
        }

        Note workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (Note)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            noteEntityCopy = (Note) noteEntity.copy();
            noteEntityCopy.setEntityTag(noteEntity.getEntityTag());
            noteEntityCopy.setOldEntity(noteEntity);
            noteEntityCopy.setEditLink((noteEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            noteEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, noteEntityCopy);
    ...
    private boolean onSaveItem() {
        ...
        ((NoteSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(noteEntityCopy);
        } else {
            viewModel.update(noteEntityCopy);
        }
        return true;
    }

    /**
     * Create a new Note instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new Note instance
     */
    private Note createNote() {
        Note noteEntity = new Note(true);
        return noteEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<Note> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(noteEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentNotesetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setNote(noteEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isNoteValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_note);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.note.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<Note> result) {
        String errorMessage;
