...
import com.sap.copy.databinding.FragmentNotificationheadersetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing NotificationHeader entity.
 * This fragment is contained in the {@link NotificationHeaderSetActivity}.
 */
public class NotificationHeaderSetCreateFragment extends InterfacedFragment<NotificationHeader> {

    private static final Logger LOGGER = LoggerFactory.getLogger(NotificationHeaderSetCreateFragment.class);
    ...
    /** NotificationHeader object and it's copy: the modifications are done on the copied object. */
    private NotificationHeader notificationHeaderEntity;
    private NotificationHeader notificationHeaderEntityCopy;

    /** DataBinding generated class */
    private FragmentNotificationheadersetCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** NotificationHeader ViewModel */
    ...
    /**
     * This fragment is used for both update and create for NotificationHeaderSet to enter values for the properties.
     ...
     *            NotificationHeader if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.notificationHeader.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.notificationHeader.getLocalName();
        }

        ((NotificationHeaderSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            notificationHeaderEntity = createNotificationHeader();
        } else {
            notificationHeaderEntity = viewModel.getSelectedEntity().getValue();
        }

        NotificationHeader workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (NotificationHeader)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            notificationHeaderEntityCopy = (NotificationHeader) notificationHeaderEntity.copy();
            notificationHeaderEntityCopy.setEntityTag(notificationHeaderEntity.getEntityTag());
            notificationHeaderEntityCopy.setOldEntity(notificationHeaderEntity);
            notificationHeaderEntityCopy.setEditLink((notificationHeaderEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            notificationHeaderEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, notificationHeaderEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isNotificationHeaderValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((NotificationHeaderSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(notificationHeaderEntityCopy);
        } else {
            viewModel.update(notificationHeaderEntityCopy);
        }
        return true;
    }

    /**
     * Create a new NotificationHeader instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new NotificationHeader instance
     */
    private NotificationHeader createNotificationHeader() {
        NotificationHeader notificationHeaderEntity = new NotificationHeader(true);
        return notificationHeaderEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<NotificationHeader> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(notificationHeaderEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentNotificationheadersetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setNotificationHeader(notificationHeaderEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isNotificationHeaderValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_notificationheader);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.notificationHeader.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<NotificationHeader> result) {
        String errorMessage;
