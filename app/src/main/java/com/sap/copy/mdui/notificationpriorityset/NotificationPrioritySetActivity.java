 ...
 * - {@link NotificationPrioritySetCreateFragment}: to create or edit a(n) NotificationPriority,
 * - {@link NotificationPrioritySetDetailFragment}: to display the details of a(n) NotificationPriority,
 * - {@link NotificationPrioritySetListFragment}: to list NotificationPrioritySet.
 *
 ...
 * what to do.
 */
public class NotificationPrioritySetActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<NotificationPriority> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link NotificationPrioritySetListFragment} is
     * getting instantiated (which will load {@link NotificationPrioritySetDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new NotificationPrioritySetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable NotificationPriority entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        NotificationPrioritySetCreateFragment fragment = new NotificationPrioritySetCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable NotificationPriority entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new NotificationPrioritySetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new NotificationPrioritySetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable NotificationPriority entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link NotificationPrioritySetCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((NotificationPrioritySetActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
    ...
    public static class DeleteConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), R.style.AlertDialogStyle));
            NotificationPriorityViewModel viewModel = new ViewModelProvider(getActivity()).get(NotificationPriorityViewModel.class);
