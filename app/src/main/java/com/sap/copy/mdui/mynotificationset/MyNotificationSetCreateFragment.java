...
import com.sap.copy.databinding.FragmentMynotificationsetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing MyNotification entity.
 * This fragment is contained in the {@link MyNotificationSetActivity}.
 */
public class MyNotificationSetCreateFragment extends InterfacedFragment<MyNotification> {

    private static final Logger LOGGER = LoggerFactory.getLogger(MyNotificationSetCreateFragment.class);
    ...
    /** MyNotification object and it's copy: the modifications are done on the copied object. */
    private MyNotification myNotificationEntity;
    private MyNotification myNotificationEntityCopy;

    /** DataBinding generated class */
    private FragmentMynotificationsetCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** MyNotification ViewModel */
    ...
    /**
     * This fragment is used for both update and create for MyNotificationSet to enter values for the properties.
     ...
     *            MyNotification if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.myNotification.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.myNotification.getLocalName();
        }

        ((MyNotificationSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            myNotificationEntity = createMyNotification();
        } else {
            myNotificationEntity = viewModel.getSelectedEntity().getValue();
        }

        MyNotification workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (MyNotification)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            myNotificationEntityCopy = (MyNotification) myNotificationEntity.copy();
            myNotificationEntityCopy.setEntityTag(myNotificationEntity.getEntityTag());
            myNotificationEntityCopy.setOldEntity(myNotificationEntity);
            myNotificationEntityCopy.setEditLink((myNotificationEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            myNotificationEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, myNotificationEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isMyNotificationValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((MyNotificationSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(myNotificationEntityCopy);
        } else {
            viewModel.update(myNotificationEntityCopy);
        }
        return true;
    }

    /**
     * Create a new MyNotification instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new MyNotification instance
     */
    private MyNotification createMyNotification() {
        MyNotification myNotificationEntity = new MyNotification(true);
        return myNotificationEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<MyNotification> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(myNotificationEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentMynotificationsetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setMyNotification(myNotificationEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isMyNotificationValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_mynotification);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.myNotification.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<MyNotification> result) {
        String errorMessage;
