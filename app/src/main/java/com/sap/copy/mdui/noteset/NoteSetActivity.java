 ...
 * - {@link NoteSetCreateFragment}: to create or edit a(n) Note,
 * - {@link NoteSetDetailFragment}: to display the details of a(n) Note,
 * - {@link NoteSetListFragment}: to list NoteSet.
 *
 ...
 * what to do.
 */
public class NoteSetActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<Note> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link NoteSetListFragment} is
     * getting instantiated (which will load {@link NoteSetDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new NoteSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable Note entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        NoteSetCreateFragment fragment = new NoteSetCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable Note entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new NoteSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new NoteSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable Note entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link NoteSetCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((NoteSetActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
