 ...
 * - {@link NotificationHeaderSetCreateFragment}: to create or edit a(n) NotificationHeader,
 * - {@link NotificationHeaderSetDetailFragment}: to display the details of a(n) NotificationHeader,
 * - {@link NotificationHeaderSetListFragment}: to list NotificationHeaderSet.
 *
 ...
 * what to do.
 */
public class NotificationHeaderSetActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<NotificationHeader> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link NotificationHeaderSetListFragment} is
     * getting instantiated (which will load {@link NotificationHeaderSetDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new NotificationHeaderSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable NotificationHeader entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        NotificationHeaderSetCreateFragment fragment = new NotificationHeaderSetCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable NotificationHeader entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new NotificationHeaderSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new NotificationHeaderSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable NotificationHeader entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link NotificationHeaderSetCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((NotificationHeaderSetActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
    ...
    public static class DeleteConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), R.style.AlertDialogStyle));
            NotificationHeaderViewModel viewModel = new ViewModelProvider(getActivity()).get(NotificationHeaderViewModel.class);
