 ...
 * - {@link ContactSetCreateFragment}: to create or edit a(n) Contact,
 * - {@link ContactSetDetailFragment}: to display the details of a(n) Contact,
 * - {@link ContactSetListFragment}: to list ContactSet.
 *
 ...
 * what to do.
 */
public class ContactSetActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<Contact> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link ContactSetListFragment} is
     * getting instantiated (which will load {@link ContactSetDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new ContactSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable Contact entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        ContactSetCreateFragment fragment = new ContactSetCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable Contact entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new ContactSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new ContactSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable Contact entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link ContactSetCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((ContactSetActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
