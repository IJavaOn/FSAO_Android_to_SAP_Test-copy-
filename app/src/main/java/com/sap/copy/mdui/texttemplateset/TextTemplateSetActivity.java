 ...
 * - {@link TextTemplateSetCreateFragment}: to create or edit a(n) TextTemplate,
 * - {@link TextTemplateSetDetailFragment}: to display the details of a(n) TextTemplate,
 * - {@link TextTemplateSetListFragment}: to list TextTemplateSet.
 *
 ...
 * what to do.
 */
public class TextTemplateSetActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<TextTemplate> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link TextTemplateSetListFragment} is
     * getting instantiated (which will load {@link TextTemplateSetDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new TextTemplateSetListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable TextTemplate entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        TextTemplateSetCreateFragment fragment = new TextTemplateSetCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable TextTemplate entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new TextTemplateSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new TextTemplateSetDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable TextTemplate entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link TextTemplateSetCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((TextTemplateSetActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
    ...
    public static class DeleteConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), R.style.AlertDialogStyle));
            TextTemplateViewModel viewModel = new ViewModelProvider(getActivity()).get(TextTemplateViewModel.class);
