...
import com.sap.copy.databinding.FragmentDatemonitorsetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing DateMonitor entity.
 * This fragment is contained in the {@link DateMonitorSetActivity}.
 */
public class DateMonitorSetCreateFragment extends InterfacedFragment<DateMonitor> {

    private static final Logger LOGGER = LoggerFactory.getLogger(DateMonitorSetCreateFragment.class);
    ...
    /** DateMonitor object and it's copy: the modifications are done on the copied object. */
    private DateMonitor dateMonitorEntity;
    private DateMonitor dateMonitorEntityCopy;

    /** DataBinding generated class */
    private FragmentDatemonitorsetCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** DateMonitor ViewModel */
    ...
    /**
     * This fragment is used for both update and create for DateMonitorSet to enter values for the properties.
     ...
     *            DateMonitor if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.dateMonitor.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.dateMonitor.getLocalName();
        }

        ((DateMonitorSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            dateMonitorEntity = createDateMonitor();
        } else {
            dateMonitorEntity = viewModel.getSelectedEntity().getValue();
        }

        DateMonitor workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (DateMonitor)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            dateMonitorEntityCopy = (DateMonitor) dateMonitorEntity.copy();
            dateMonitorEntityCopy.setEntityTag(dateMonitorEntity.getEntityTag());
            dateMonitorEntityCopy.setOldEntity(dateMonitorEntity);
            dateMonitorEntityCopy.setEditLink((dateMonitorEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            dateMonitorEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, dateMonitorEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isDateMonitorValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((DateMonitorSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(dateMonitorEntityCopy);
        } else {
            viewModel.update(dateMonitorEntityCopy);
        }
        return true;
    }

    /**
     * Create a new DateMonitor instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new DateMonitor instance
     */
    private DateMonitor createDateMonitor() {
        DateMonitor dateMonitorEntity = new DateMonitor(true);
        return dateMonitorEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<DateMonitor> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(dateMonitorEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentDatemonitorsetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setDateMonitor(dateMonitorEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isDateMonitorValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_datemonitor);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.dateMonitor.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<DateMonitor> result) {
        String errorMessage;
