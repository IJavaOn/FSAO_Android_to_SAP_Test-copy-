package com.sap.copy.mdui.pmuserdetailsset;
...
import com.sap.copy.databinding.FragmentPmuserdetailssetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing PMUserDetails entity.
 * This fragment is contained in the {@link PMUserDetailsSetActivity}.
 */
public class PMUserDetailsSetCreateFragment extends InterfacedFragment<PMUserDetails> {

    private static final Logger LOGGER = LoggerFactory.getLogger(PMUserDetailsSetCreateFragment.class);
    ...
    /** PMUserDetails object and it's copy: the modifications are done on the copied object. */
    private PMUserDetails pMUserDetailsEntity;
    private PMUserDetails pMUserDetailsEntityCopy;

    /** DataBinding generated class */
    private FragmentPmuserdetailssetCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** PMUserDetails ViewModel */
    ...
    /**
     * This fragment is used for both update and create for PMUserDetailsSet to enter values for the properties.
     ...
     *            PMUserDetails if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.pmUserDetails.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.pmUserDetails.getLocalName();
        }

        ((PMUserDetailsSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            pMUserDetailsEntity = createPMUserDetails();
        } else {
            pMUserDetailsEntity = viewModel.getSelectedEntity().getValue();
        }

        PMUserDetails workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (PMUserDetails)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            pMUserDetailsEntityCopy = (PMUserDetails) pMUserDetailsEntity.copy();
            pMUserDetailsEntityCopy.setEntityTag(pMUserDetailsEntity.getEntityTag());
            pMUserDetailsEntityCopy.setOldEntity(pMUserDetailsEntity);
            pMUserDetailsEntityCopy.setEditLink((pMUserDetailsEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            pMUserDetailsEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, pMUserDetailsEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isPMUserDetailsValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((PMUserDetailsSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(pMUserDetailsEntityCopy);
        } else {
            viewModel.update(pMUserDetailsEntityCopy);
        }
        return true;
    }

    /**
     * Create a new PMUserDetails instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new PMUserDetails instance
     */
    private PMUserDetails createPMUserDetails() {
        PMUserDetails pMUserDetailsEntity = new PMUserDetails(true);
        return pMUserDetailsEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<PMUserDetails> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(pMUserDetailsEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentPmuserdetailssetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setPMUserDetails(pMUserDetailsEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isPMUserDetailsValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_pmuserdetails);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.pmUserDetails.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<PMUserDetails> result) {
        String errorMessage;
